...
function currentModeIndex(state){
  return Math.max(0, Math.min(3, (Number(state.mode)||1)-1));
}

function measuredAt(state,i){
  const c=currentModeIndex(state);
  const v=state.vals[i] ? state.vals[i][c] : null;
...
function calcSeries(state){
 ...
 for(let i=0;i<state.N;i++){
   ...
   data.push({p:i+1,y:Number.isFinite(cu)?cu:NaN});
...
function calcDeviation(data,state){
 ...
 if(!ps||!pe||s===e){
   return{maxDev:0,devPoint:null,line:null,left:{...blank},center:{...blank},right:{...blank}};
 }

 const lineYAt=p=>ps.y+(pe.y-ps.y)*(p-s)/(e-s);
 const sections={
   left:{maxDev:0,devPoint:null},
   center:{maxDev:0,devPoint:null},
   right:{maxDev:0,devPoint:null}
 };

 for(const d of data){
   if(!Number.isFinite(d.y))continue;
   const ly=lineYAt(d.p);
   const dv=Math.abs(d.y-ly);
   const key=d.p<s?"left":(d.p>e?"right":"center");
   if(dv>sections[key].maxDev){
     sections[key]={maxDev:dv,devPoint:{p:d.p,y:d.y,lineY:ly,dev:dv}};
   }
 }
 ...
 for(const k of ["center","right"]){
   if(sections[k].maxDev>maxObj.maxDev)maxObj=sections[k];
 }
