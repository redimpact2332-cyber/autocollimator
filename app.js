...
function redraw(){
 window.graphInvertY=state.invertY===true;
 window.graphInvertX=state.invertX===true;
 if(result) drawGraph($("graph"),result.data,result.dev,state.N);
 const pg=$("printGraph");
 if(pg&&result) drawGraph(pg,result.data,result.dev,state.N);
}

function cloneVals(){return state.vals.map(r=>r.slice())}
function pushUndo(){undoStack.push({vals:cloneVals(),N:state.N,rangeStart:state.rangeStart,rangeEnd:state.rangeEnd,tolerance:state.tolerance,invertY:state.invertY,invertX:state.invertX}); if(undoStack.length>50)undoStack.shift(); redoStack=[]}
function restore(s){state.vals=s.vals.map(r=>r.slice());state.N=s.N;state.rangeStart=s.rangeStart;state.rangeEnd=s.rangeEnd;state.tolerance=s.tolerance||0;state.invertY=!!s.invertY;state.invertX=!!s.invertX;normalize();buildTable();update(false)}
...
function addGraphControlButtons(){
 if($("invertYBtn"))return;
 const graphCard=$("graph").closest(".card");
 const box=document.createElement("div");
 box.className="rowBtns";
 box.style.marginBottom="8px";
 box.innerHTML=`
  <button id="invertYBtn">＋−上下反転</button>
  <button id="invertXBtn">左右反転</button>
  <span class="note">グラフ表示だけ反転します。計算値は変わりません。</span>
 `;
 graphCard.insertBefore(box,graphCard.firstChild);
...
function update(doSave=true){
 ...
 for(const r of result.rows){
  for(let c=1;c<=4;c++)$("v"+c+"_"+r.pos).textContent=fmt(r.vals[c-1]);
  $("m_"+r.pos).textContent=fmt(r.m);
  $("co_"+r.pos).textContent=fmt(r.co);
  $("cu_"+r.pos).textContent=fmt(r.cu);
 }
 redraw();
 render();
 if(doSave)save();
...
function showShot(){
 ...
 redraw();$("printTable").innerHTML=`<table>${$("dataTable").innerHTML}</table>`;scrollTo({top:0});
}
...
buildTable();update(false);renderHistory();
 ...
 $("undoBtn").onclick=()=>{const s=undoStack.pop();if(s){redoStack.push({vals:cloneVals(),N:state.N,rangeStart:state.rangeStart,rangeEnd:state.rangeEnd,tolerance:state.tolerance,invertY:state.invertY,invertX:state.invertX});restore(s)}};$("redoBtn").onclick=()=>{const s=redoStack.pop();if(s){undoStack.push({vals:cloneVals(),N:state.N,rangeStart:state.rangeStart,rangeEnd:state.rangeEnd,tolerance:state.tolerance,invertY:state.invertY,invertX:state.invertX});restore(s)}};
